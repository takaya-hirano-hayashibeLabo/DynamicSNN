...
def norm_mse(x_base,x):
    """
    baseで正規化してからMSEを取る関数
    """
    x_max=np.max(x_base)
    x_min=np.min(x_base)

    x_base_norm=(x_base-x_min)/(x_max-x_min)
    x_norm=(x-x_min)/(x_max-x_min)

    mse=np.mean((x_base_norm-x_norm)**2)

    return mse


def plot_batch_element(out_v_ideal, out_v_scaled, n, saveto, head_t=50, window_size=5):
    """
    Plot the n-th batch element of out_v_ideal and out_v_scaled, including their moving averages.
    ...
    for i in range(xdim):
        ...
        ideal_moving_avg = np.convolve(out_v_ideal[n, :, i].cpu().numpy(), np.ones(window_size)/window_size, mode='valid')
        scaled_moving_avg = np.convolve(out_v_scaled[n, :, i].cpu().numpy(), np.ones(window_size)/window_size, mode='valid')
        
        ax3.plot(np.array(range(len(ideal_moving_avg)))[head_t:], ideal_moving_avg[head_t:], label=f'xdim {i}')
        ax4.plot(np.array(range(len(scaled_moving_avg)))[head_t:], scaled_moving_avg[head_t:], label=f'xdim {i}')

        print(f"xdim:{i} mes: {norm_mse(ideal_moving_avg,scaled_moving_avg)}")
    ...
    ax3.set_title(f'Moving Average of out_v_ideal - Batch element {n}')
    ax3.set_xlabel('T')
    ax3.set_ylabel('Value')
    ax3.legend()
    
    ax4.set_title(f'Moving Average of out_v_scaled - Batch element {n}')
    ax4.set_xlabel('T')
...
def plot_base_sclaed_volt(out_v_base, out_v_scaled, n, saveto, window_size=5):
    """
    Plot the n-th batch element of out_v_base and out_v_scaled, including their moving averages.
    
    :param out_v_base: [N x T x xdim]
    :param out_v_scaled: [N x T x xdim]
    :param n: index of the batch element to plot
    :param window_size: size of the moving average window
    """
    T_base = out_v_base.shape[1]
    T_scaled=out_v_scaled.shape[1]
    xdim = out_v_base.shape[2]

    fig, axes = plt.subplots(3, 1, figsize=(12, 16))
    ax1, ax2, ax3= axes
    
    for i in range(xdim):
        ax1.plot(np.array(range(T_base)), out_v_base[n, :, i].cpu().numpy(), label=f'xdim {i}')
        ax2.plot(np.array(range(T_scaled)), out_v_scaled[n,:, i].cpu().numpy(), label=f'xdim {i}')
        
        # Calculate moving averages
        scaled_moving_avg = np.convolve(out_v_scaled[n, :, i].cpu().numpy(), np.ones(window_size)/window_size, mode='valid')
        ax3.plot(np.array(range(len(scaled_moving_avg))), scaled_moving_avg, label=f'xdim {i}')
    
    ax1.set_title(f'out_v_base - Batch element {n}')
    ax1.set_xlabel('T')
    ax1.set_ylabel('Value')
    ax1.legend()
    ...
    ax3.set_title(f'Moving Average of out_v_scaled - Batch element {n}')
    ax3.set_xlabel('T')
    ax3.set_ylabel('Value')
    ax3.legend()
...
def main():
    ...
    parser.add_argument("--testsize",default=-1,type=int,help="-1だと全部")
    ...
    with torch.no_grad():
        ...
        for inputs, targets in tqdm(test_loader_base):
            ...
            if not encoder_conf is None:
                inputs=encoder(inputs) # [N x T x xdim]
            else:
                inputs[inputs>0]=1.0

            inputs=inputs.permute((1,0,*[i+2 for i in range(inputs.ndim-2)])) # [T x N x xdim]
