...
def plot_results(results_db, saveto: Path, fig_conf: dict):
    ...
    for model in unique_models:
        ...
        ax.plot(
            ...
            marker=get_marker(model_data, fig_conf),
            label=model, color=cmp(fig_conf["model-color"]["alpha"] * fig_conf["model-color"][model.split("_")[0]]),
    ...
    ax.set_xlabel('Time Scale'   ,fontsize=fig_conf["labelsize"])
    ...
    # モデルの種類ごとのlegendを追加
    tau_legend = ax.legend(handles=tau_legend_handles, bbox_to_anchor=(0.5, 1.1), loc='upper center', frameon=False, ncol=3,fontsize=fig_conf["legend"]["fontsize"])
    ax.add_artist(tau_legend)

    # tauのサイズごとのlegendを追加
    ax.legend(handles=model_legend_handles, bbox_to_anchor=(0.5, 1.175), loc='upper center', frameon=False, ncol=4,fontsize=fig_conf["legend"]["fontsize"])
    ...
    fig.savefig(savefile, bbox_inches='tight')  # Figureの外枠を削除
    plt.close(fig)
