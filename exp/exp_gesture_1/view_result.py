...
def plot_results(results_db,saveto:Path):
    ...
    for model in unique_models:
        ...
        plt.plot(model_data["time-scale"], model_data["acc-mean"], '-o', label=model)
        plt.fill_between(model_data["time-scale"], 
                         model_data["acc-mean"] - model_data["acc-std"], 
                         model_data["acc-mean"] + model_data["acc-std"], 
                         alpha=0.2)

    plt.title('Accuracy vs Time-Scale')
...
def main():
    ...
    for file in dirs:
        ...
        try:
            ...
            results.append(
                [modeltype,js["time-scale"],js["acc_mean"],js["acc_std"]]
            )

        except:
            pass

    results_db=pd.DataFrame(results,columns=["model-type","time-scale","acc-mean","acc-std"])
