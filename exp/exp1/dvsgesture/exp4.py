...
def main():
    ...
    relativepath="20241120.dynasnn.exp4.multiA/"
    ...
    with torch.no_grad():
        ...
        lif_states=model.test_dynamic_forward_multi_a_with_lifstate(
            scaled_in.to(device).to(torch.float),
            a=list(np.array([0.816506391,0.869017942,0.924875654,0.930950194,0.954996858,0.920567885,1.026188555,0.857807462,0.775462957]))
            )
