...
def main():
    ...
    savepath=Path(__file__).parent / f"result_reverse/trial_{args.trial}"
    ...
    a=1 # ここは絶対値にする(=マイナスをつけない)
    # Create scaled_input by shifting indices by a factor of 'a'
    scaled_input = torch.zeros(size=(abs(int(a * T)), batch, insize))
    for t in range(T):
    ...
    scaled_input = torch.flip(scaled_input, [0]) #時間方向に反転


    org_s,org_i,org_v=model.forward(scaled_input)
    scaled_s,scaled_i,scaled_v=model.dynamic_forward_v1(scaled_input,a=torch.Tensor([-1*a for _ in range(scaled_input.shape[0])]))
