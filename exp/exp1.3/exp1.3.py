...
def main():
    ...
    for time_window in range(100,1000,100):
            ...
            in_max=np.max(base_in)
            print(f"base in max: {in_max}")
            # base_in=base_in/in_max
            base_in[base_in>0]=1.0
            # base_in[base_in>=thre]=1
            # base_in[base_in<thre]=0
            ...
            # in_real[in_real>=thre]=1
            # in_real[in_real<thre]=0
            in_real=torch.Tensor(in_real).to(device).permute(1,0,2,3,4)
            with torch.no_grad():
            ...
            # with torch.no_grad():
            #     s_ideal,v_ideal=model.dynamic_forward_v1(
            #         in_ideal,
            #         a=[a for _ in range(in_real.shape[0])]
            #     )
