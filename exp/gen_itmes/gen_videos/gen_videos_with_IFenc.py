...
def main():
    ...
    threshold=10
    encoder=IFEncoder(threshold=threshold,device=device)

    timescale=args.timescale
    time_window=int(3000/timescale)

    savepath=Path(__file__).parent/f"IFEnc_thr{threshold}_window{time_window}-insize{args.insize}"
    ...
    with torch.no_grad():
        for inputs, targets in tqdm(test_loader):
            inputs, targets = inputs.to(device).to(torch.float), targets.to(device) #[batch x T x xdim]
