...
def main():
    ...
    parser.add_argument("--sequence_size",type=int,default=500,help="全体の時系列長さ")
    ...
    rate_high,rate_low,rate_none=0.8,0.3,0.01 #high->1, low->0, none->空
    rate_pattern=[
    ...
    for pattern in rate_pattern:
        ...
        rate_list=[r for r in pattern["in"]]

        input_spikes=[]
        for iT in range(3): #前半入力フレーム・空フレーム・後半入力フレームの3つの区間に分ける

            in_iT=torch.where(
                torch.rand(size=(T_list[iT],int(datasize/4),in_size))<rate_list[iT], #一様分布でスパイク生成
                1.0,0.0
            )
            input_spikes.append(in_iT)
