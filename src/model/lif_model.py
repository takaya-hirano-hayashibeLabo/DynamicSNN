...
class DynamicLIF(nn.Module):
    ...
    def __init__(
        ...
        #>> リカレント接続重み >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
        # 参考 式19 [https://arxiv.org/pdf/2210.03515]
        # self.wh=nn.Linear(in_size[0],in_size[0],bias=False)
    ...
    def forward(self,current:torch.Tensor):
        ...
        # h=self.wh.forward(self.spike_prev) if not self.spike_prev is None else 0.0  #リカレント状態
        # if self.output:
        #     print("sum(h): ",torch.sum(h).item(), ", sum(sp prev): ", torch.sum(self.spike_prev).item()) if not self.spike_prev is None else print("sp prev : ", self.spike_prev)
