...
from model.encoder import IFEncoder
...
def main():
    ...
    #>> 閾値エンコーダ >>
    if_enc=IFEncoder(threshold=10) #入力は [N x T x xdim]
    #>> 閾値エンコーダ >>
    ...
    save_heatmap_video(event2frame(xout1).to("cpu").detach().numpy(),output_path=Path(__file__).parent,file_name=f"xout_a{a1}",frame_label_view=False)
    save_heatmap_video(event2frame(xout2).to("cpu").detach().numpy(),output_path=Path(__file__).parent,file_name=f"xout_a{a2}",frame_label_view=False)
