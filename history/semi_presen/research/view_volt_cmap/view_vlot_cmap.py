...
from src.model import DynamicCSNN,CSNN,DynamicResCSNN,MultiLayerDynamicResCSNN,ResCSNN,CSNN
...
def main():
    ...
    modelname=re.sub("(.)*laplace-version/","",args.target)
    ...
    cachepath=str(f"/mnt/ssd1/hiranotakaya/master/dev/workspace/prj_202409_2MR/laplace-version/exp/exp_gesture_1/test-cache/gesture-window{time_window}")
    ...
    testset =  tonic.datasets.DVSGesture(save_to=str(datapath),  train=is_train_data,transform=transform)
    # cachepath=ROOT/f"cache-data/gesture/window{time_window}-insize{insize}"
    cachepath=str(f"/mnt/ssd1/hiranotakaya/master/dev/workspace/prj_202409_2MR/laplace-version/exp/exp_gesture_1/test-cache/gesture-window{int(time_window/a)}")
    if os.path.exists(cachepath):
        testset=tonic.DiskCachedDataset(testset,cache_path=str(cachepath))
