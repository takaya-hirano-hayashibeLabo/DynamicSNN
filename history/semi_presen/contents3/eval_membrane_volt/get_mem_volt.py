...
def main():
    ...
    parser.add_argument("--scaletype",default="real")
    ...
    resdir=Path(__file__).parent/f"{args.saveto}/results"
    ...
    # print(config)
    if "encoder" in config.keys() and config["encoder"]["type"].casefold()=="if":
        threshold=config["encoder"]["threshold"]
        if_encoder=IFEncoder(threshold=threshold)
    else:
        if_encoder=None
    print(f"if_encoder: {if_encoder}")

    datapath=ROOT/"original-data"

    time_window_key = "timewindow" if "timewindow" in config["train"].keys() else "time-window"
    time_window=config["train"][time_window_key]
    insize=config["model"]["in-size"]
    batch_size=config["train"]["batch"]
    time_sequence=config["train"]["sequence"]
    ...
    if if_encoder is None:
        base_in[base_in>0]=1.0 #スパイククリップ
    else:
        base_in=if_encoder(base_in) #encoder
    ...
    a=float(args.timescale)

    #>> 実際のタイムスケール倍 >>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>>
    if args.scaletype=="real":
        transform=torchvision.transforms.Compose([
            tonic.transforms.Denoise(filter_time=10000),
            tonic.transforms.Downsample(sensor_size=tonic.datasets.DVSGesture.sensor_size,target_size=(insize,insize)),
        ...
        if if_encoder is None:
            ...
            scaled_in=if_encoder(scaled_in) #encoder
    ...
    plot_results(
        ...
        v2=scaled_v,
        v3=org_v,
        filename=resdir/f"mem_volt_type{args.scaletype}"
    )
